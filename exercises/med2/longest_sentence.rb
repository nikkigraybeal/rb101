...
def longest_sentence(text)
  ...
  arr.each_index do |index|
    current = arr[index].split(' ').size
    if current > longest
      longest = current
      longest_index = index
    end
  end
  puts "The longest sentence is #{longest} words long."
  puts arr[longest_index]
end

def longest_word(text)
  words = text.split(/[^a-zA_Z]/)
  longest = 0
  longest_word = ' '
  words.each_index do |index|
    current = words[index].size
    if current > longest
      longest = current
      longest_word = words.at(index)
    end
  end
  puts "The longest word is #{longest_word} and it is #{longest} characters."
end
