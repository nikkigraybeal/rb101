...
egyptian
input: a fraction
output: an array of denominators representing the unit fractions that can be summed to equal the original input fraction

problem: break down the fraction into unit fractions

data structure: empty array to push unit fraction denominators to

numerator * unit is bigger than denominator?
   subtract unit fraction from fraction
   push unit denominator to new array
  increment unit denominator
  repeat on resulting fraction beginning with next unit fraction until numerator of the resulting fraction == 1
...
def egyptian(fraction)
  ...
  loop do
    ...
    if (fraction.numerator * unit) > fraction.denominator
      fraction -= Rational(1, unit)
...
def unegyptian(array)
  sum = 0
  array.each{|unit| sum += Rational(1, unit)}
  sum
end
